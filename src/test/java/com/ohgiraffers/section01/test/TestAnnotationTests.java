...
public class TestAnnotationTests {
    ...
    * -Junit3, 4를 기반으로 돌아가는 테스트 엔진을 제공해준다.
    ...
    테스트 클래스는 abstract이면 안되고 한개 이상의 생성자를 가지고 있어야한다.
    (두개 이상 작성하면 런타임시 PreconditionViolationException 이 발생합니다.)
    아무런 생성자도 작성하지 않으면 기본 생성자는 컴파일러가 자동으로 추가합니다.
     */
    ...
    public void 테스트_메소드2(){
        /*
        클래스 레벨에 @DisplayNameGernerator 를 붙이게 되면 언더바를 공백으로 처리하여 테스트 이름을 바꿔줄수있다.
